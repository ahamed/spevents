...
class SpeventsControllerMedia extends JControllerAdmin
{
	...
	public function getModel($name = 'Media', $prefix = 'SpeventsModel', $config = array())
	{
    ...
    public function upload_media() {
        ...
        $thumbsize  = $input->post->get('thumbsize', '300x225', 'STRING');
        $report     = array();
        ...
        if(count($image)) {
            if ($image['error'] == UPLOAD_ERR_OK) {
                ...
                if (($postMaxSize > 0 && $contentLength > $postMaxSize) || ($memoryLimit != -1 && $contentLength > $memoryLimit)) {
                    $report['status'] = false;
                    $report['output'] = JText::_('SPMEDIA_MANAGER_MEDIA_TOTAL_SIZE_EXCEEDS');
                    $error = true;
                ...
                if (($image['error'] == 1) || ($uploadMaxSize > 0 && $image['size'] > $uploadMaxSize) || ($uploadMaxFileSize > 0 && $image['size'] > $uploadMaxFileSize)) {
                    $report['status'] = false;
                    $report['output'] = JText::_('SPMEDIA_MANAGER_MEDIA_LARGE');
                    $error = true;
                }
                ...
                if(!$error) {
                    ...
                    if(JFile::upload($path, $dest)) {
                        ...
                        if(strtolower($ext) == 'svg') {
                            ...
                            $image = new SpmediaHelperImage($dest);
                            if( ($image->getWidth() >300) || ($image->getWidth() >225) ) {
                                $image->createThumbs(array('spmedia_thumb'=>$thumbsize), 5, '_spmedia_thumbs');
            ...
            $report['status'] = false;
            $report['output'] = JText::_('SPMEDIA_MANAGER_UPLOAD_FAILED');
        }
        echo json_encode($report);
        die();
    }
    ...
    public function delete_media() {
        ...
        if(!is_numeric($id)) {
            $report['status'] = false;
            $report['output'] = JText::_('SPMEDIA_MANAGER_DELETE_FAILED');
            echo json_encode($report);
            die;
        }
