...
if($saveOrder) {
	$saveOrderingUrl = 'index.php?option=com_spevents&task=sessions.saveOrderAjax&tmpl=component';
...
<form action="<?php echo JRoute::_('index.php?option=com_spevents&view=sessions'); ?>" method="POST" name="adminForm" id="adminForm">
	...
	<div id="j-main-container" class="span10" >
		...
		<pre class="text-center"><code>No data!</code></pre>
		<?php else: ?>
		<table class="table table-striped" id="eventList">
			...
			<tr>
				...
				<th>
					<?php echo JText::_('COM_SPEVENTS_EVENTS_TITLE_LABEL'); ?>
				</th>
				<th>
					<?php echo JHtml::_('grid.sort','Date','a.date',$listDirn,$listOrder); ?>
				</th>
				<th>
					<?php echo JHtml::_('grid.sort','Hall','a.hall',$listDirn,$listOrder); ?>
				</th>
			...
			<?php foreach($this->items as $i => $item): ?>
				...
				<tr class="row<?php echo $i % 2; ?>" sortable-group-id="1">
					...
					<td>
						...
						<?php if ($canEdit) : ?>
							<a class="sp-events-title" href="<?php echo JRoute::_('index.php?option=com_spevents&task=session.edit&id='.$item->id);?>">
					...
					<td>
						<?php echo EH::belongsTo('#__spevents_sessions', '#__spevents_events', 'id', 'event_id', $item->event_id)->title; ?>
					</td>
					<td class="">
						<?php echo $item->date; ?>
						<small><?php echo "(Time range: " . $item->time_from . " - " . $item->time_to . ")"; ?></small>
					</td>
					<td class="">
						<?php echo $item->hall; ?>
					</td>
