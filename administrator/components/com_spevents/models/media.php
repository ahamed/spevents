...
class SpeventsModelMedia extends JModelList
{
	...
	public function insertMedia($title, $path, $thumb='', $type='image') {
		...
		$values = array($db->quote($title), $db->quote($path), $db->quote($thumb), $db->quote($type), $db->quote($title), $db->quote($extension), $db->quote( JFactory::getDate('now') ), JFactory::getUser()->id);
		$query
